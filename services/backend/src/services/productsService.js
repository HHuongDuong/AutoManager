module.exports = function createProductsService(deps) {
  ...
  async function listProducts(filter) {
    const { branchFilter, branch_id, category_id, q } = filter;
  ...
  async function updateProduct(id, payload) {
    const { category_id, sku, name, price, metadata, image_url } = payload;
    const result = await db.query(
      ...
      [id, category_id ?? null, sku ?? null, name ?? null, price ?? null, image_url ?? null, metadata ?? null]
    );
    return result.rows[0] || null;
  }
